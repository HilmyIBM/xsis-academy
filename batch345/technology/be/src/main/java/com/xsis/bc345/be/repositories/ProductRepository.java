...
public interface ProductRepository extends JpaRepository <Product, Integer> {
    ...
    // +"INNER JOIN tbl_m_categories AS c ON v.category_id = c.id "
    // +"WHERE v.is_deleted IS NOT TRUE AND LOWER(p.name) LIKE CONCAT('%', :filter, '%')", nativeQuery = true)
    // Optional <List<Map<String, Object>>> updateProduct(@Param ("name") String name, @Param ("price") double price, @Param ("image") String image, @Param ("stock") int stock, @Param ("variantId") int variantId);
}

...
public interface CategoryRepository extends JpaRepository<Category, Integer> {
        ...
        Optional<List<Category>> findByDeletedAndCategoryNameContainingIgnoreCaseOrDescriptionContainingIgnoreCase(
                        boolean deleted,
                        String categoryName,
                        String description);
        ...
        @Query(value = "SELECT c.id, "
                        + "c.description, c.category_name AS \"categoryName\", c.is_deleted AS \"deleted\", "
                        + "c.create_by AS \"createBy\", c.create_date AS \"createDate\", "
                        + "c.update_by AS \"updateBy\", c.update_date AS \"updateDate\" "
                        + "FROM tbl_m_categories AS c WHERE c.is_deleted = FALSE AND "
                        + "(LOWER(c.category_name) LIKE LOWER(CONCAT('%', :filter, '%')) "
                        + "OR LOWER(c.description) LIKE LOWER(CONCAT('%', :filter, '%')))", nativeQuery = true)
        Optional<List<Category>> findCategoryNameOrDescription(@Param("filter") String filter);
}

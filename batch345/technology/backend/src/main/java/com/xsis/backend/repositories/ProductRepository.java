...
public interface ProductRepository extends JpaRepository<Product, Integer> {
    ...
    @Query(value = "SELECT p.id, p.name, c.category_name AS \"categoryName\", v.name AS \"variantName\", "
            ...
            + "WHERE (p.is_deleted = FALSE AND c.is_deleted = FALSE AND v.is_deleted = FALSE) AND p.id = :id", nativeQuery = true)
    Optional<Map<String, Object>> findNativeById(@Param("id") int id);
    ...
    @Query(value = "SELECT p.id, p.name, c.category_name AS \"categoryName\", v.name AS \"variantName\", p.price, p.stock, p.variant_id AS \"variantId\", "
            ...
            + "WHERE p.is_deleted = FALSE AND c.is_deleted = FALSE AND v.is_deleted = FALSE", nativeQuery = true)
    ...
    @Query(value = """
            SELECT
                p.id, p.name, c.category_name AS "categoryName", v.name AS "variantName", p.price,
                p.stock, p.variant_id AS "variantId", p.image,
            ...
            FROM tbl_m_product AS p
                ...
                INNER JOIN tbl_m_categories AS c ON v.category_id = c.id
            WHERE (p.is_deleted = FALSE AND c.is_deleted = FALSE AND v.is_deleted = FALSE) AND
                (LOWER(p.name) LIKE LOWER(CONCAT('%', :filter, '%')) OR
                LOWER(c.category_name) LIKE LOWER(CONCAT('%', :filter, '%')) OR
                LOWER(v.name) LIKE LOWER(CONCAT('%', :filter, '%')))
            """, nativeQuery = true)

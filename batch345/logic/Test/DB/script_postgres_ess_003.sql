...
SELECT 
   ...
   age(lr.start_date - lr.end_date) AS total_leave_days
FROM 
    leave_request lr
INNER JOIN 
...
--3
SELECT CONCAT(b.first_name, ' ', b.last_name) AS nama_lengkap, P.name, count(F.status)
FROM employee E
inner join employee_position EP on E.id = EP.employee_id
inner join position P on EP.position_id  = P.id
inner join biodata B on E.biodata_id = B.id
inner join family F on B.id = F.biodata_id
having F.status  = 'Anak'
group by nama_lengkap , P.name;

--4
SELECT  B.first_name || ' ' || B.last_name Nama_Lengkap, B.pob ,B.address 
FROM biodata B
WHERE B.pob = 'Jakarta' AND address NOT ILIKE '%Jakarta%';

--5Berapakah jumlah masing-masing karyawan kontrak dan permanen
select CONCAT(b.first_name, ' ', b.last_name) AS nama_lengkap, count(e.status)
from employee e
inner join biodata b on e.biodata_id = b.id 
group by E.status, nama_lengkap

--6Tampilkan nama karyawan, jenis perjalanan dinas, tanggal perjalanan dinas, dan total pengeluarannya selama perjalanan dinas tersebut,

select 
b.first_name || ' ' || b.last_name AS nama_lengkap,
tt.name status,
...
--7
SELECT 
    CONCAT(b.first_name, ' ', b.last_name) nama_lengkap,
    d.name "Departemen"
FROM 
    biodata b
INNER JOIN 
    employee e ON b.id = e.biodata_id
INNER JOIN 
    employee_position ep ON e.id = ep.employee_id
INNER JOIN 
    department d ON ep.position_id = d.id;

--8Buatklah query untuk menampilkan nama lengkap karyawan, status pernikahan, nama pasangan(suami/istri) yang sudah memiliki anak
SELECT 
    CONCAT(b.first_name, ' ', b.last_name) nama_lengkap,
    CASE 
        ...
        ELSE 'Belum Menikah'
    END status,
    f.name 	nama_pasangan
FROM 
    biodata b
INNER join
	family f on b.id= f.biodata_id
where f.status = 'Suami'
group by nama_lengkap, status, nama_pasangan,b.marital_status

--9Tampilkan data biodata yang tidak memiliki posisi kepegawaian
select 
 	b.id id,
    CONCAT(b.first_name, ' ', b.last_name) nama_lengkap,
    b.pob tempat_lahir,

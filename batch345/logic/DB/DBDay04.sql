...
--No.1
SELECT nama,COUNT(cust_id) as order from sales_person 
INNER JOIN orders on sales_person.id=orders.sales_person_id
GROUP BY nama
HAVING COUNT(cust_id) >1; 

--No.2
SELECT nama,SUM(amount) as "Total Orders" from sales_person 
join orders on sales_person.id=orders.sales_person_id
GROUP BY nama
HAVING SUM(amount) >1000; 

--No.3
SELECT nama,EXTRACT(YEAR from Age(sales_person.bod)) as umur,salary,SUM(amount) As "Total Amount" 
from sales_person join orders on sales_person.id=orders.sales_person_id
WHERE EXTRACT(YEAR from orders.order_date) >=2020
GROUP BY nama,sales_person.bod,salary
ORDER BY umur ASC;

--No.4
SELECT nama, AVG(orders.amount) AS "Rata - Rata" 
from sales_person JOIN orders

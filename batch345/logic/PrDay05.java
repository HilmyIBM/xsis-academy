...
public class PrDay05 {
    ...
    public static void SOS(String sinyal) {
        int sinyal_salah = 0;
        String pola = "SOS";
        for (int i = 0; i < sinyal.length(); i += 3) {
            for (int j = 0; j < 3 && i + j < sinyal.length(); j++) {
                if (sinyal.charAt(i + j) != pola.charAt(j)) {
                    sinyal_salah++;
                }
            }
        }
        System.out.println("Total Sinyal salah: " + sinyal_salah);
    }
}
